...
def jarvis_loop(pause_event, registry, args):
    ...
    while True:
        ...
        direct_commands = [
            ...
            "play", "show", "find", "tell", "do", "get", "set", "turn", "run",
            "close", "shut", "exit", "quit", "send", "message", "email",
            "take", "capture", "screenshot", "help", "assist",
            "stop", "pause", "mute", "increase", "decrease",
            "type", "press", "keyboard", "clear", "submit", "enter"
        ]
        
        is_direct = any(cmd in user_query for cmd in direct_commands)
        
        try:
            # STRICT MODE: Only execute if there's a command keyword or wake word
            # No auto-interpretation, no options - ONLY what user commands
            if is_direct:
                # Has a command keyword - execute it
                clean_query = user_query.replace("jarvis", "").strip()
                response = jarvis.run_conversation(clean_query)
            elif "jarvis" in user_query:
                # Has wake word - process the command
                clean_query = user_query.replace("jarvis", "").strip()
                response = jarvis.run_conversation(clean_query)
            else:
                # No command keyword and no wake word - IGNORE (strict mode)
                continue
...
def main():
    ...
    # 4. Start GUI in Main Thread (Required for PyQt) - skip in text mode
    if not args.text:
        run_gui_app(pause_event)
    else:
        # In text mode, keep main thread alive
        try:
            while True:
                time.sleep(1)
